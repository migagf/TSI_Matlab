...
if plotting == 1
    figure(1)
    plot(LWheel_geom_Tr(:,1)+R_track(1),LWheel_geom_Tr(:,2)+R_track(2),'b',LRail_geom_Tr(:,1)+R_track(1),LRail_geom_Tr(:,2)+R_track(2),'k',RWheel_geom_Tr(:,1)+R_track(1),RWheel_geom_Tr(:,2)+R_track(2),'b',RRail_geom_Tr(:,1)+R_track(1),RRail_geom_Tr(:,2)+R_track(2),'k'), grid on, set(gca, 'YDir','reverse')%, axis([-2 2 -0.5 0.2])
    drawnow
end
