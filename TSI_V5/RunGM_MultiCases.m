...
SP_vals = [0.01 40 80 120];  % Speed of train
SF_vals = [0.21 0.74 1.0 1.38 2.0 5.0];  % Hazard Cases
...
for SP_index = 2:3 %4                  % Speed
for GM_index = 1:length(AllGMs)     % GM index
for SF_index = 1:5                  % Scale Factor
for CF_index = 1:2                  % Coupling Factor
for NL_index = 1:2                  % Linear/Nonlinear

AnalysisCode = strcat('Res_',num2str(SP_index),'_',num2str(GM_index,"%02.f"),'_',num2str(SF_index),'_',num2str(CF_index),'_',num2str(NL_index));
...
for it = 2:tsteps
    ...
    % Save contact forces on each wheel
    QL(it) = F(2, 1);
    QR(it) = F(2, 2);
    YL(it) = F(1, 1);
    YR(it) = F(1, 2);
