...
if plotting == 1
    plot(LWheel_geom_Tr(:,1) + R_track(1), LWheel_geom_Tr(:,2) + R_track(2), 'k', ...
        ...
        RRail_geom_Tr(:,1)+R_track(1),RRail_geom_Tr(:,2)+R_track(2),'k'), grid on, set(gca, 'YDir','reverse'), axis([-5 5 -10 0]), xlabel('$X$ (m)'), ylabel('$Y$ (m)') %, ylim([-0.4 -0.1])%axis([-1.5 1.5 -0.5 0.0])
